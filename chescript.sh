...
sub_resume(){
    ...
    kubectl rollout status -w -n che deployment/che

    if [ $? -eq 0 ]; then
        echo
        echo "Installation completed."
        echo "Eclipse Che should be up on https://che-che.$hostaddress.nip.io/"
        echo "You may need to install the CA certificate file for the TLS certificate,"
        echo "available in /tmp/vagrant/cheCA.crt (on a Linux host) - retrieve its content with:"
        echo ' vagrant ssh -c "cat cheCA.crt"'
        echo 'or copy/paste the following:'
        cat /home/vagrant/cheCA.crt
    fi
...
sub_start(){
    ...
    sudo ./chectl-install.sh 2>/dev/null
...
EOF
        ...
        sudo chectl server:start --platform minikube --installer=helm --multiuser --domain $hostaddress.nip.io --workspace-pvc-storage-class-name=workspace-storage --postgres-pvc-storage-class-name=postgres-storage
    else
    ...
    if [ $? -eq 0 ]; then
        cp /home/vagrant/cheCA.crt /home/vagrant/data/
